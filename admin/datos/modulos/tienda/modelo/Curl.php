...
class Curl{
	...
	public function buildPostData($data){
		...
		if(
			...
			preg_match($this->jsonPattern, $this->headers['Content-Type']) && (is_array($data) || (is_object($data) &&
		...
		if (
			!$binary_data && (is_array($data) || is_object($data)) && (!isset($this->headers['Content-Type']) ||
	...
	public function exec($ch = null){
		...
		if ($this->curlError && function_exists('curl_strerror')) {
			$this->curlErrorMessage =
				curl_strerror($this->curlErrorCode) . (empty($this->curlErrorMessage) ? '' : ': ' . $this->curlErrorMessage);
		}
		...
		if ($follow_303_with_post) {
			...
			if (isset($this->options[CURLOPT_CUSTOMREQUEST])) {
				if ((version_compare(PHP_VERSION, '5.5.11') < 0) || defined('HHVM_VERSION')) {
					trigger_error(
						'Debido a las limitaciones técnicas de PHP <= 5.5.11 y HHVM, no es posible '
						. 'realizar una solicitud post-redirect-get usando un objeto Curl php-curl-class que'
						. 'ya se ha utilizado para realizar otro tipo de solicitudes. Utilice un nuevo '
						. 'php-curl-class Objeto Curl o actualice su motor PHP.',
						E_USER_ERROR
					);
				} else {
	 ...
	 * @return mixed Returns the value provided by exec.
	 */
	public function binary($url, $data = ''){
		if (is_array($url)) {
			$data = $url;
			$url = (string) $this->url;
		}
	 ...
	 * @param  $password
	 */
	public function setNTLMAuthentication($username, $password = ''){
		$this->setOpt(CURLOPT_HTTPAUTH, CURLAUTH_NTLM);
		$this->setOpt(CURLOPT_USERPWD, $username . ':' . $password);
	}
	...
	public function getCookies(){
		...
		$cookie = $headers->offsetGet("cookie");
		if (!empty($cookie)) {
			return $cookie;
		}
		return NULL;
	...
	public function setDefaultUserAgent(){
		$user_agent = 'Curl-PHP/' . self::VERSION . ' (+12';
	...
	private function parseHeaders($raw_headers){
		$raw_headers = !empty($raw_headers) ? preg_split('/\r\n/', $raw_headers) : [];
