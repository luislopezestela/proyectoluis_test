...
$cant_ventas=DatosAdmin::ventas_registrados($_SESSION['admin_id']);
if($cant_ventas >0){
...
if($_POST['new_devolutions_init']=="create_devolucion"){
	$venta->id_vendedor = $_SESSION['admin_id'];
	$venta->fecha = date("Y-m-d H:i:s");
	$venta->crear_devolucion();
	echo json_encode(array('estado'=> true));
}


if($_POST['new_devolutions_init']=="add_cliente"){
	$venta->id_vendedor = $_SESSION['admin_id'];
	$venta->id_cliente = $_POST['dat'];
	$venta->fecha = date("Y-m-d H:i:s");
	$venta->cliente_devolucion_item();
	echo json_encode(array('estado'=> true));
}

if($_POST['new_devolutions_init']=="documents"){
	$venta->vendedor = $_SESSION['admin_id'];
	$venta->documento=$_POST['doc'];
	$venta->actualiza_documento_devolucion();
	echo json_encode(array('estado'=> true));
}

if($_POST['new_devolutions_init']=="add_num_doc") {
	$venta->vendedor = $_SESSION['admin_id'];
	$venta->numero_doc = $_POST['doc'];
	$venta->actualiza_num_doc_devolucion();
	echo json_encode(array('estado'=> true));
}

if($_POST['new_devolutions_init']=="add_metodo_order"){
...
if($_POST['new_devolutions_init']=="add_items_in_order"){
